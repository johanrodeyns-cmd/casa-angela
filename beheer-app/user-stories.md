...
> Vervolg in v0.46.0: op vraag van Johan ("zo weinig mogelijk API-calls, maximaal Firebase") leest deze grafiek nu rechtstreeks uit de permanente `energyHistory`-archivering (US-6.11) i.p.v. nog een live APsystems-call te doen — `logic.nutsCacheTtlMs` is daarmee overbodig geworden en verwijderd. Bijwerking: Dag/Maand tonen nu data t.e.m. gisteren i.p.v. t.e.m. vandaag (de huidige, nog niet afgesloten dag verschijnt pas na de eerstvolgende nachtelijke archivering) — bewust aanvaard als eenvoudigere en robuustere aanpak dan een extra live call voor enkel de vandaag-waarde.
> Vervolg in v0.48.0: op vraag van Johan kan bij "Maand" nu tussen jaren gewisseld worden (◀/▶, `#nuts-year-nav`), zelfde patroon als Water/Gas' jaarnavigatie (`chartYear`-state + prev/next-knoppen). `logic.buildMonthlySeriesFromArchive` ondersteunde al een `year`-parameter, dus geen nieuwe pure functie nodig — enkel `buildArchiveSeriesAndLabels(level, field, year)` in `index.html` kreeg het jaartal doorgegeven i.p.v. steeds `new Date().getFullYear()` te hardcoden. Geldt ook voor Netstroom (US-6.10), zelfde gedeelde functie.
...
- Given "Dag", then toont de grafiek altijd alle dagen van de huidige maand, ook de nog niet verstreken dagen (als 0 kWh) — sinds v0.46.0 incl. de dagen tussen gisteren en het einde van de maand.

**Technische notities (sinds v0.46.0):** geen `casaAngelaEnergy`-call meer — `loadEnergyHistory()` (`index.html`) laadt eenmalig per sessie de volledige `energyHistory`-collectie, `logic.buildArchiveSeriesAndLabels(level, "solar")` (intern: `logic.getArchiveDailySeries`/`buildMonthlySeriesFromArchive`/`buildYearlySeriesFromArchive`, alle pure/TDD) bouwt de reeks + labels. `logic.padSeriesValues(values, length)` + `logic.daysInMonth(year, month)` vullen Dag aan tot een vaste lengte (Maand/Jaar hebben dat niet meer nodig, want `buildMonthlySeriesFromArchive` levert altijd 12 waarden en `buildYearlySeriesFromArchive` enkel jaren met data).
...
> Vervolg in v0.46.0: sinds de historie-grafieken (US-6.4/US-6.10) rechtstreeks uit `energyHistory` lezen i.p.v. via `nutsCached`, blijft deze cache enkel nog gebruikt voor de twee live "vandaag"-databronnen (`today_{date}`, `meter_energy_hourly_{date}`) — geen `energy_{level}_{range}`/`meter_energy_{level}_{range}`-sleutels meer.
...
> Vervolg in v0.46.0: de Dag/Maand/Jaar-historie leest nu, net als Zonnestroom (US-6.4), rechtstreeks uit de permanente `energyHistory`-archivering i.p.v. `casaAngelaMeterEnergy` met level daily/monthly/yearly aan te roepen — geen APsystems-call en geen EID meer nodig voor de historie. "Netstroom vandaag" blijft ongewijzigd (`level: "hourly"`, live, intraday wordt niet gearchiveerd).
...
> Vervolg in v0.47.0: Johan wilde de eerste run niet tot 03:00 's nachts moeten afwachten om te zien dat het werkt. Nieuwe onCall `casaAngelaArchiveEnergyRunNow` (`requireAllowedUser`) roept dezelfde `runEnergyArchive()` aan als de cron (uit `casaAngelaArchiveEnergy` getrokken, gedeelde functie — zelfde patroon als `processNutsMonitor`/`casaAngelaMonitorRunNow`); knop "Forceer archivering nu" (Zonnestroom > Instellingen) roept 'm aan en herlaadt daarna `loadEnergyHistory(force=true)` + de Archief-status + beide historie-grafieken, zodat het resultaat meteen zichtbaar is zonder de pagina te moeten herladen.
> Vervolg in v0.47.1: op de vraag "van wanneer tot wanneer is er gearchiveerd?" bleek de statusregel enkel het vage "tot nu" te tonen, niet de effectieve laatst gearchiveerde dag. Elk maand-document krijgt nu een `throughDate`-veld (`logic.archiveThroughDate(yyyymm, todayIso)` in `functions/lib/energyArchive.js`, pure/TDD — laatste dag van de maand als die al voorbij is, anders "gisteren" via Date-aritmetiek zodat een maandwissel correct verloopt, `null` als er nog geen enkele dag van die maand gearchiveerd is). De statusregel toont voortaan "... tot en met JJJJ-MM-DD" i.p.v. "... tot nu".
> Vervolg in v0.47.2 (bugfix): eerste geforceerde run (via de v0.47.0-knop) archiveerde niets — `casaAngelaArchiveEnergyRunNow`-logs toonden voor zowel zonnestroom als netstroom een niet-JSON HTTP 500 op elke geprobeerde maand. Bij het naspeuren van de officiële "APsystems OpenAPI User Manual" (§3.4.2) bleek de netstroom/meter-endpoint al die tijd het verkeerde pad-namespace te gebruiken: `/installer/api/v2/systems/{sid}/devices/meter/period/{eid}` i.p.v. het gedocumenteerde `/user/api/v2/...`. `/installer/api/v2/...` bestaat wel in de API, maar uitsluitend voor de storage/battery-endpoints. Verklaart met terugwerkende kracht ook een eerder losstaand genoteerd "minutely geeft HTTP 500"-probleem op diezelfde endpoint (zie technische notities hieronder) — beide zijn hetzelfde onderliggende padprobleem. Rechtgezet in zowel `casaAngelaMeterEnergy` als `archiveMonth`; zonnestroom gebruikte al het juiste `/user/api/v2/systems/energy/{sid}`-pad, dus als dat na deze fix nog steeds faalt is dat een apart (vermoedelijk tijdelijk, extern) probleem.
...
**Technische notities:** `casaAngelaArchiveEnergy` (`onSchedule`, dagelijks 03:00 Spaanse tijd, `timeoutSeconds: 300`) — leest credentials uit `settings/apsystems` (zelfde als de bestaande Nuts-functies), roept per maand zowel `casaAngelaEnergy`'s onderliggende endpoint (zonnestroom, `user/api/v2/systems/energy`) als `casaAngelaMeterEnergy`'s endpoint (netstroom, `user/api/v2/.../meter/period` — was tot v0.47.2 foutief `installer/api/v2`, zie vervolgnotitie hierboven) aan met `energy_level=daily`, en schrijft beide dagreeksen samen naar `energyHistory/{YYYY-MM}`. Terugwaartse opbouw: loopt maand per maand terug (max. 60 per run) tot een reeds-compleet Firestore-document (normale situatie na de eerste keer) of een APsystems-foutcode 1001 ("geen data beschikbaar", vóór de meter-installatie — bewaard in `energyHistory/_meta.earliestAvailableMonth` zodat latere runs die grens niet blijven herbevragen). Pure helpers in `functions/lib/energyArchive.js` (`daysInMonth`, `previousYyyyMm`, `isMonthComplete`, `padMonthArray`), TDD-getest in `functions/test/energyArchive.test.js` — eigen kopie i.p.v. hergebruik van `logic.js`, zelfde reden als de iCal-parser (Cloud Functions deployen enkel de `functions/`-map). `firestore.rules`: `energyHistory/{docId}` enkel leesbaar voor de twee toegelaten gebruikers, nooit client-schrijfbaar (enkel de Cloud Function via de admin-SDK schrijft).

---

### US-6.12 ☑ Zonnestroom/Netstroom-historie en gemiddeld-verbruik-berekening zonder herhaalde API-calls (M) — v0.46.0
> Aanleiding: op vraag van Johan ("zo weinig mogelijk API-calls, maximaal Firebase") is de bestaande Dag/Maand/Jaar-historie (US-6.4/US-6.10) herbouwd bovenop het archief uit US-6.11, en is de berekeningslaag voor "gemiddeld dagverbruik per gast" (besproken 2026-07-21) meteen mee klaargezet — de visualisatie daarvan volgt in een aparte, nog te bespreken stap.

**Als** Johan of Tinneke **wil ik** dat het tonen van zonnestroom-/netstroomgegevens (bestaande historie én straks het gemiddeld verbruik per gast) zo weinig mogelijk APsystems-calls kost en zoveel mogelijk uit Firebase komt.

**Acceptatiecriteria:**
- Given de Zonnestroom- of Netstroom-historie (Dag/Maand/Jaar), then komt de data volledig uit `energyHistory` (Firestore) — geen APsystems-call meer, ongeacht hoe vaak de tab bezocht wordt.
- Given "Vermogen vandaag"/"Netstroom vandaag", then blijven die live via APsystems (intraday-detail wordt niet gearchiveerd) — enige overgebleven live databron.
- Given een boeking uit het verleden, then kan het gemiddeld dagverbruik (netstroom, kWh/dag) berekend worden uit enkel `energyHistory`-data, zonder APsystems-call.

**Technische notities:** `index.html`: `loadEnergyHistory()` laadt de volledige `energyHistory`-collectie + `_meta` eenmalig per sessie (zelfde "eenmalig bij login"-patroon als `loadBookings`/`loadSyncedBlocks`); `renderNutsHistory`/`renderNetstroomHistory` gebruiken `buildArchiveSeriesAndLabels(level, field)` i.p.v. `casaAngelaEnergyFn`/`casaAngelaMeterEnergyFn`; `renderNutsArchiveStatus()` (US-6.11-vervolg) hergebruikt dezelfde geladen data i.p.v. een eigen Firestore-read. Nieuwe pure functies in `logic.js` (TDD): `getArchiveDailySeries`, `buildMonthlySeriesFromArchive`, `buildYearlySeriesFromArchive`, `sumArchiveDateRange`, `averageDailyConsumption(booking, docsByMonth)` — de laatste is de berekeningslaag voor het gemiddeld verbruik per gast (`sumArchiveDateRange(..., 'grid') / nightsBetween(...)`, `null` bij 0 nachten), klaar om aan een UI te koppelen zodra de visualisatie is vastgelegd. `logic.nutsCacheTtlMs` is hiermee overbodig en verwijderd; `nutsCached`/`cache/apsystems` (30 min TTL) blijft enkel bestaan voor de twee live "vandaag"-databronnen. **Bijwerking**: Dag/Maand-historie toont nu data t.e.m. gisteren i.p.v. t.e.m. vandaag (de archivering loopt 's nachts) — bewust aanvaard i.p.v. een extra live call voor enkel vandaags waarde.

**Nog open (bewust niet in deze story)**: hoe het gemiddeld verbruik per gast in de UI getoond wordt (bv. in de boekingenlijst, een dag-detail-dialoog, of iets anders) — apart te bespreken met Johan.
