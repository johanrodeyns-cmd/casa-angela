...
> Vervolg in v0.39.0: extra grafiek "Netstroom vandaag" (per-minuut geëxporteerde energie, volledige dag), analoog aan Zonnestroom's "Vermogen vandaag".
...
- Given diezelfde sub-tab, then toont die ook een "Netstroom vandaag"-grafiek (per-minuut geëxporteerde energie, volledige dag 00:00-23:59) — exact hetzelfde patroon als "Vermogen vandaag" bij Zonnestroom.

**Technische notities:** Cloud Function `casaAngelaMeterEnergy` (onCall) — GET naar `/installer/api/v2/systems/{sid}/devices/meter/period/{eid}` (Meter-level Data API, zelfde `buildHeaders`-ondertekening als de bestaande Nuts-calls, enkel een ander pad-namespace dan `/user/api/v2/...`) — geeft `{time, produced, consumed, imported, exported}`-arrays terug, waarvan enkel `exported` gebruikt wordt. `renderNetstroomHistory(level)` is vrijwel een letterlijke kopie van `renderNutsHistory`, incl. hergebruik van `logic.padSeriesValues`/`logic.daysInMonth`. Cloud Function `casaAngelaMeterToday` (v0.39.0, mirror van `casaAngelaToday`) — zelfde endpoint met `energy_level=minutely`, geeft `data.energy.exported` terug (geen apart exported-vermogen beschikbaar, enkel een gecombineerd netto import/export-vermogen); `renderNetstroomTodayChart()` hergebruikt `logic.padTodaySeries` (hernoemd van `padTodayPowerSeries`, want nu ook voor kWh i.p.v. enkel W). Geen eigen Firestore-collectie of -document nodig (geen `netstroomReadings`, geen `settings/emaSession`).
